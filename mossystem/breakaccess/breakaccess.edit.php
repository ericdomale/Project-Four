...
$query = "SELECT * FROM breakthrough WHERE id='$id'";
...
function load_names(){
    $conn = mysqli_connect('127.0.0.1:3307','root','','mossystem');
    $output = '';
    $sql = "SELECT * FROM membership  WHERE cell= 'Breakthrough' ORDER BY memberno";
...
<head>
    ...
    <link rel="stylesheet" href="breakaccess.css">
...
<body>
     ...
     <header>
       ...
       <h2 class="heading">BREAKTHROUGH</h2>
       <nav>
           <ul class="nav_links">
     ...
     <?php 
                    if (isset($_POST['edited']))
                    {
                        ...
                        $conn = mysqli_connect("127.0.0.1:3307", "root", "", "mossystem"); 
                        $query = "UPDATE `breakthrough` SET entrydate='$_POST[entrydate]',connumber='$_POST[connumber]',choristers='$_POST[choristers]',preacher='$_POST[preacher]',message='$_POST[message]',ofamount='$_POST[ofamount]',members='$_POST[members]',connames='$_POST[connames]',attended='$attended' WHERE id='$id'";
                        $queryrun = mysqli_query($conn,$query);

                         if ($queryrun)
                        { /*
                             $_SESSION['success'] ="Details Updated";
                             header('location:breakthrough.php');
                         }else
                         {
                        ...
                        echo "<script>window.location.href='breakaccess.php'</script>";
...
<script src="breakaccess.js"></script>
